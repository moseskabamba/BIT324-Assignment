...
<div>
    ...
    <div class="w-full flex space-x-4 justify-context">
     @foreach ($products as $product)
        <div class="flex flex-col md:flex-wrap md:flex-row p-5">


                <div class="w-full md:w-1/2 lg:w-1/3 px-4 md:px-2 py-2">
                    <div class="bg-white rounded shadow p-5 h-full relative">
                        <h5 class="font-black uppercase text-2xl mb-4">
                            {{ $product->name ?? 'no products uploaded' }}
                        </h5>
                        <h6 class="font-bold text-gray-700 text-xl mb-3">K {{ $product->price }}</h6>
                        <p class="text-gray-900 font-normal mb-12">
                           <a target="_blank" href="/storage/public/{{ $product->product->image ?? "" }}">
                                <img class="w-full h-full hover:scale-110 transition duration-300 ease-in-out" src="{{ asset($product->product->image ?? 'img/logo.png') }}" alt="">
                            </a>
                        </p>
